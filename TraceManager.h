...
//      std::cerr << std::endl << "Computing inside weight summand" << std::endl;
        const auto & rule_weight  = boost::get<Trainer::RuleTensorRaw<double, rule_rank>>(rules[edge->get_label_id()]);

//        std::cerr << "rule tensor " << edge->get_label_id() << " address " << rules[edge->get_label_id()] << std::endl << rule_weight << std::endl;
//        std::cerr << "target weight " << target_weight << std::endl;
...
//        std::cerr << "rule tensor " << edge->get_original_id() << " address " << rules[edge->get_original_id()] << std::endl << rule_weight << std::endl;
        ...
        const auto & item_weight = inside_weights.at(edge->get_sources()[rhs_pos-1]);
...
//        std::cerr << "rule tensor " << edge->get_original_id() << " address " << rules[edge->get_original_id()] << std::endl << rule_weight << std::endl;
    ...
    inline void outside_weight_step2(
            const std::vector<Trainer::RuleTensor<double>>&rules
            , const MAPTYPE<Element<Node<Nonterminal>>, Trainer::WeightVector>& outside_weights
            , Trainer::WeightVector & outside_weight
        ...
        const auto & rule_weight = boost::get<Trainer::RuleTensorRaw <double, rule_rank>>(rules[outgoing.first->get_label_id()]);
    ...
    inline void outside_weight_step3(
        ...
        const auto & rule_weight = boost::get<Trainer::RuleTensorRaw<double, rule_rank>>(rules[outgoing.first->get_label_id()]);
        const auto & parent_weight = outside_weights.at(parent);
        const auto & rhs_weight = inside_weights.at(siblings[position == 0 ? 1 : 0]);
...
//        std::cerr << "rule_val" << 0 << std::endl<< rule_val << std::endl << std::endl;
        ...
        for (unsigned rhs_pos = 1; rhs_pos < rule_rank; ++rhs_pos) {
            ...
            const auto & item_weight = inside_weights.at(siblings[rhs_pos - 1]);
...
//        split_merge_cycle(N_THREADS, BATCH_SIZE, cycle, n_epochs, epsilon, merge_threshold, merge_percentage, grammar_Info.rule_to_nonterminals, grammar_Info.normalizationGroups, grammar_Info.nont_idx, nont_dimensions, rule_weights_val, root_weights);
...
//        const std::vector<std::vector<unsigned>> & normalization_groups = grammarInfo.normalizationGroups;
//
