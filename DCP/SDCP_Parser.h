...
namespace std {
    template<typename Nonterminal, typename Position>
    struct hash<DCP::ParseItem<Nonterminal, Position>> {
        //    typedef ParseItem<Nonterminal, Position>> argument_type;
        //    typedef std::size_t result_type;

        size_t operator()(DCP::ParseItem<Nonterminal, Position> const &item) const {
            size_t seed = 0;
...
namespace DCP {
    ...
    template<typename Nonterminal, typename Position>
    std::ostream &operator<<(std::ostream &os, DCP::ParseItem<Nonterminal, Position> const &item) {
        os << item.nonterminal << " ( ";
    ...
    private:
            ...
            for (TermOrVariable<Terminal> obj : sterm) {
                if (obj.type() == typeid(Term<Terminal>)) {
                    ...
                    if (term.children.size() > 0) {
                        ...
                        if (!match_and_retrieve_vars(
                                input.get_children(position).front()
                                , term.children
                                , var_assignment
                                , dummy
                                , lcfrs_terminals
        ...
        bool match_lcfrs(
                const Rule <Nonterminal, Terminal> &rule
                , const std::vector<Position> &lcfrs_terminals
        ...
        void match_lexical_rules() {
            ...
            for (const auto &pair : input.terminals()) {
                ...
                for (std::shared_ptr<Rule<Nonterminal, Terminal>> rule : sDCP.get_axioms(terminal)) {
                    ...
                    std::vector<Position> lcfrs_terminals;
                    if (match_and_retrieve_vars(
                            input.get_previous(position)
                            , rule->inside_attributes[0][0]
                            , var_assignment
                            , span_assignment
                            , lcfrs_terminals
                    )) {
                        ...
                        agenda.push(item);
                        trace[*item].push_back(
                                std::make_pair(
                                        rule, std::vector<std::shared_ptr<ParseItem<Nonterminal
                                                                                    , Position>>
                                                         >()));
        ...
        bool match_sterm_rec(
                STerm <Terminal> sterm, Position pos
...
//                            return false;
                    ...
                    if (var_.member > 0) {
                        if (lhn_var) {
                            ...
                            inherited[var.argument - 1].second = pos;
                            if (debug)
                                std::cerr << " match var " << var << " <" << inherited[var.argument - 1].first << "-"
                                          << pos << ">" << std::endl;
                            lhn_var = false;
...
//                    }
    ...
    public:
        ...
        MAPTYPE<ParseItem<Nonterminal, Position>, std::vector<std::pair<std::shared_ptr<Rule < Nonterminal, Terminal>>
                                                              , std::vector<std::shared_ptr<ParseItem<Nonterminal
                                                                                                      , Position>> >>>> trace;
        ...
        SDCPParser(
                bool parse_lcfrs = false
                , bool debug = false
                , bool no_parallel = true
                , bool inh_strict_successor = true
        )
                : no_parallel(no_parallel), inh_strict_successor(inh_strict_successor), debug(debug),
        ...
        void add_recursively(
                std::unordered_set<ParseItem<Nonterminal, Position>> &reachable, ParseItem<Nonterminal
                                                                                           , Position
                                                                                          > &start
        ) const {
        ...
        void reachability_simplification() {
            ...
            if (this->goal) {
                ...
                MAPTYPE<ParseItem<Nonterminal, Position>, std::vector<std::pair<std::shared_ptr<Rule<Nonterminal
                                                                                                     , Terminal>>
        ...
        query_trace(
        ParseItem<Nonterminal, Position> start
        ) const {
            std::vector<std::pair<Rule < Nonterminal, Terminal>, std::vector<ParseItem<Nonterminal, Position>>>>
            result;
